...
const TAG = mad_logs_1.buildFileTag('user-model.ts', colors.bgBlue.white);
const users = [];
/****************************************** CREATE USER *******************************************/
class UserModel {
    constructor(opts) {
        ...
        this.comparePassword = (password) => new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            return (isMatch
                ? resolve(true)
                : reject(new error_objects_1.LoginError(`${TAG} Incorrect password`, __filename, this.username)));
        }));
        this.save = () => {
    ...
    static findOne(args) {
        return new Promise((resolve, reject) => {
            ...
            return reject(new error_objects_1.LoginError(`${TAG} UsersMock.Users array doesn't contain user`, 'user-model.ts', args.username || ''));
        });
    }
}
