...
function log() {
    return function (target, name) {
        ...
        target[name] = function (...args) {
            ...
            console.log(`%c>> FUNCTION ${name} CALLED${(target.name) ? ('on ' + target.name) : ''} >>`, wrapCallHeaderStyle);
            if (args.length > 0) {
            ...
            console.log(result);
            console.log('%c' + _.pad(` END FUNCTION ${name} OUTPUT `, outputHeader.length, '>'), wrapCallHeaderStyle);
            console.log('%c' + _.repeat('<', 150), wrapCallHeaderStyle);
            return result;
        };
