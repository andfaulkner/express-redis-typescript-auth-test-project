...
// Set the paths being monitored for changes. Trigger tests when their contents change.
let watchDir = 'build/**/*.js';
if (process.env.CLIENT_ONLY) watchDir = 'build/client/**/*.js'
if (process.env.SERVER_ONLY) watchDir = 'server/client/**/*.js'
...
Module.prototype.require = function(path) {
    ...
    // Mimics Webpack's "alias" feature     <== use whenever needed
    // if ( path === 'config' ) {
    //   path = resolve( './build/js/secrets/test.js' );
    // }
...
chokidar.watch(watchDir, { persistent: true })
    .on('change', path => runSuite())
